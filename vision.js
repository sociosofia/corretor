...
const colU={A:(180-20)/1160,B:(335-20)/1160,C:(487-20)/1160,D:(639-20)/1160,E:(793-20)/1160};
const rowV={1:(82-20)/500,2:(138-20)/500,3:(196-20)/500,4:(254-20)/500,5:(311-20)/500,6:(369-20)/500,7:(425-20)/500,8:(483-20)/500};
...
function bubbleScores(img,c){const W=img.width,H=img.height,d=img.data,topW=Math.hypot(c[1].x-c[0].x,c[1].y-c[0].y),botW=Math.hypot(c[3].x-c[2].x,c[3].y-c[2].y),radius=Math.max(4,Math.round(((topW+botW)/2)*.0112)),out={};for(let q=1;q<=8;q++){out[q]={};for(const l of'ABCDE'){const p=bilerp(c,colU[l],rowV[q]);let n=0,dark=0,sat=0;const x0=Math.max(0,Math.floor(p.x-radius)),x1=Math.min(W-1,Math.ceil(p.x+radius)),y0=Math.max(0,Math.floor(p.y-radius)),y1=Math.min(H-1,Math.ceil(p.y+radius));for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){const dx=x-p.x,dy=y-p.y;if(dx*dx+dy*dy>radius*radius)continue;const i=(y*W+x)*4,r=d[i],g=d[i+1],b=d[i+2];n++;if(lum(r,g,b)<160)dark++;if(rgbToSat(r,g,b)>50)sat++}out[q][l]=n?Math.max(dark/n,sat/n):0}}return out}
function classify(scores){const rows=[];for(let q=1;q<=8;q++){const s=scores[q],strong=[...'ABCDE'].filter(l=>s[l]>=VALID),susp=[...'ABCDE'].filter(l=>s[l]>=REVIEW&&s[l]<VALID);let state,answer;if(strong.length>=2){state='dupla';answer=strong.join('/')}else if(strong.length===1){state='válida';answer=strong[0]}else if(susp.length){state='revisar';answer=susp.sort((a,b)=>s[b]-s[a])[0]}else{state='em branco';answer='—'}rows.push({q,answer,state,max:Math.max(...Object.values(s))})}return rows}
